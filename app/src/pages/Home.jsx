...
function Home() {
  ...
  async function generateRecipe() {
    ...
    setIsLoading(true)
    try {
      const aiRecipe = await aiGenerateRecipe({
        prompt: prompt.trim()
      });
      setRecipe(aiRecipe.recipe);
    } catch (error) {
        ...
        {isLoading ? 
          <div className="text-center mb-12 flex flex-col items-center justify-center">
            ...
            <span className="text-white-600 text-xl font-medium">
                Your recipe is cooked...
            </span>
          </div>
        :  !recipe ? (
          <div className="text-center mb-12">
            <h1 className="text-4xl font-bold bg-gradient-to-r from-red-900 to-black-900 bg-clip-text text-transparent">
              Welcome to KukAI
            </h1>
          </div>
        ) : (
        ...
        <div className="max-w-5xl mx-auto px-4">
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-2 bg-gradient-to-r from-black-400 to-red-400 text-white rounded-md disabled:from-gray-600 disabled:to-gray-600 disabled:cursor-not-allowed transition-all duration-200 text-sm flex items-center gap-1"
            >
              {isLoading ? (
                <>
                  <svg
